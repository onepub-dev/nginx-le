...
String getClientIP({bool debug = false}) {
  var request = waitForEx<HttpClientRequest>(HttpClient().getUrl(Uri.parse(getIPURL)));
  ...
  // defer
  for (var content in waitForEx<List<String>>(response.transform(Utf8Decoder()).toList())) {
    clientIP = content;
  }
