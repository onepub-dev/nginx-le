...
void main(List<String> args) {
  ...
  final runTests = parsed['test'] as bool ? '--test' : '--no-test';

  final cliProject = DartProject.self;
  ...
  final project =
      DartProject.fromPath(join(cliProject.pathToProjectRoot, '../shared'));
...
void showUsage(ArgParser parser) {
  print('Releases all nginx-le packages to pub.dev and pushes the nginx-le '
      'docker container');
  print(parser.usage);
}
...
void build(Version? newVersion) {
  ...
  final imageTag = '$name:${newVersion.toString()}';
